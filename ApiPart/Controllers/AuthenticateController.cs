...
namespace ApiPart.Controllers
{
    ...
    public AuthenticateController(
        ...
        //Get by id
       // [HttpGet("{id}")]
        // public async Task<ActionResult<RegisterModel>> GetUsers(int id)
        //{
        ...
        public async Task<IActionResult> RegisterFreelancer([FromBody] RegisterModel model)
        {
            try
            {
                ...
                if (!result.Succeeded)
                    return StatusCode(StatusCodes.Status400BadRequest, new Response { Status = "Error", Message = "User creation failed! Please check user details and try again." });
                if (!await _roleManager.RoleExistsAsync(UserRoles.Freelancer))
                    await _roleManager.CreateAsync(new IdentityRole(UserRoles.Freelancer));
                return Ok(new Response { Status = "Success", Message = "User created successfully!" });
            }
            catch (Exception)
            {
        ...
        public async Task<IActionResult> RegisterAdmin([FromBody] RegisterModel model)
        {
            ...
            if (await _roleManager.RoleExistsAsync(UserRoles.Admin))
            {
                await _userManager.AddToRoleAsync(user, UserRoles.Freelancer);
            }
            return Ok(new Response { Status = "Success", Message = "User created successfully!" });
        }
